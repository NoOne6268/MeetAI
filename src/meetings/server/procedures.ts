...
export const meetingsRouter = createTRPCRouter({
    ...
    getTranscipt: protectedProcedure.input(z.object({
        id: z.string(),
    ...
    generateToken: protectedProcedure.mutation(async ({ ctx }) => {
        ...
        const expitationTime = Math.floor(Date.now() / 1000) + (60 * 60); // Token valid for 1 hour
    ...
    create: premiumProcedure("meeting").input(meetingsInsertSchema)
        .mutation(async ({ input, ctx }) => {
    ...
    getMany: protectedProcedure
        ...
        .query(async ({ ctx, input }) => {
            ...
            const totalPages = Math.ceil((total.count ?? 0) / pageSize) ?? 1;
